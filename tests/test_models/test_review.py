...
class Test_Review(unittest.TestCase):
    """ checks certain aspects of Review Class"""
    def test_equal(self):
        """ checks the sucess cases of Review """
        test_object = Review()
        self.assertIsInstance(test_object, Review)
        time.sleep(1)
        test_object.name = "My_First_Model"
        test_object2 = Review(**test_object.to_dict())
    ...
    def test_print(self):
        ...
        test_str = f"[Review] ({test_object.id}) {test_object.__dict__}"
        self.maxDiff = None
