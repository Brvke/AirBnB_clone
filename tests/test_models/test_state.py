...
class Test_State(unittest.TestCase):
    """ checks certain aspects of State Class"""
    def test_equal(self):
        """ checks the sucess cases of State """
        test_object = State()
        self.assertIsInstance(test_object, State)
        time.sleep(1)
        test_object.name = "My_First_Model"
        test_object2 = State(**test_object.to_dict())
    ...
    def test_print(self):
        ...
        test_str = f"[State] ({test_object.id}) {test_object.__dict__}"
        self.maxDiff = None
