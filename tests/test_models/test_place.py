...
class Test_Place(unittest.TestCase):
    """ checks certain aspects of Place Class"""
    def test_equal(self):
        """ checks the sucess cases of Place """
        test_object = Place()
        self.assertIsInstance(test_object, Place)
        time.sleep(1)
        test_object.name = "My_First_Model"
        test_object2 = Place(**test_object.to_dict())
    ...
    def test_print(self):
        ...
        test_str = f"[Place] ({test_object.id}) {test_object.__dict__}"
        self.maxDiff = None
